...
// gitignored, creating it with random values the first time. Always uses the dev sandbox SSO profile
// (COMMANDER_AWS_PROFILE overrides it), never whatever AWS_PROFILE the shell has, and the stack is
// pinned to the sandbox account so CDK refuses credentials for any other account.
...
const SANDBOX_ACCOUNT = '182399717497'; // tire-rack-dev-sandbox-developer
...
// Low before the event so testing days can't add up past the $10 envelope; raise to 3000 for 10/15
config.modelCallsPerDay ??= 300;
...
const env = { ...process.env, AWS_PROFILE: process.env.COMMANDER_AWS_PROFILE || 'tire-rack-dev-sandbox-developer', AWS_REGION: 'us-east-1' };
